...
class Market extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $raccess = $this->library->role_access('Market');
        if (isset($raccess)) {
            $this->roles = $raccess;
        }
    }
    ...
    public function index()
    {
        ...
        $data['content'] = $this->config->item('base_theme') . '/Market/Market';

        $session = array('xfield_' . $this->dtable => '', 'xsearch_' . $this->dtable => '', 'sortBy_' . $this->dtable => 'eyeMarket_id', 'sortDir_' . $this->dtable => 'desc',
        ...
        $data['count'] = $this->excurl->reqCurl('Market', array_merge($query, array('count' => true)))->data[0];
        $data['limit'] = $limit;
    ...
    function add()
    {
        if ($this->roles == 'admin' OR $this->roles->menu_created == 1) {
            $data['title'] = 'Market';
            $data['parent'] = $this->mparent;
            $data['content'] = $this->config->item('base_theme') . '/Market/add_Market';

            $data['category'] = $this->excurl->reqCurl('Market-category');

            if ($this->input->post('val') == true) {
                $this->load->view($this->config->item('base_theme') . '/Market/add_Market', $data);
            } else {
                $this->load->view($this->config->item('base_theme') . '/template', $data);
            }
        } else {
    ...
    function subcategory()
    {
        ...
        $category = $this->excurl->reqCurl('Market-category-sub', ['Market_type_id' => $search]);

        if ($category) {
            if ($category->data) {
                foreach ($category->data as $cat) {
                    echo "<option value='$cat->sub_Market_id'>$cat->sub_category_name</option>";
                }
            } else {
