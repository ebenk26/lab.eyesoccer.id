...
class News extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $raccess = $this->library->role_access('news');
        if (isset($raccess)) {
            $this->roles = $raccess;
        }
    }
    ...
    public function index()
    {
        ...
        $data['content'] = $this->config->item('base_theme') . '/news/news';

        $session = array('xfield_' . $this->dtable => '', 'xsearch_' . $this->dtable => '', 'sortBy_' . $this->dtable => 'eyenews_id', 'sortDir_' . $this->dtable => 'desc',
        ...
        $data['count'] = $this->excurl->reqCurl('news', array_merge($query, array('count' => true)))->data[0];
        $data['limit'] = $limit;
    ...
    function search()
    {
        if ($this->input->post('val') == true) {
            ...
            $data['dt'] = $this->excurl->reqCurl('news', $query['query'])->data;
            $data['count'] = $this->excurl->reqCurl('news', $query['count'])->data[0];
            $data['limit'] = $limit;
            $data['offset'] = $this->offset;
            $data['prefix'] = $this->dtable;
    ...
    function pagetable()
    {
        if ($this->input->post('val') == true) {
            ...
            $html = $this->load->view($this->config->item('base_theme') . '/news/news_table', $data, true);
    ...
    function save()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_created == 1) {
            $option = $this->excurl->reqAction('news/save', array_merge($_POST, array('ses_user_id' => $this->session->userdata('user_id'))), ['uploadfile']);
    ...
    function edit($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            if ($id == '') {
                ...
                $query = array('eyenews_id' => $id, 'detail' => true);
                $ulevel = $this->library->user_check();
                if($ulevel->ff > 0)
                {
    ...
    function update()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            $option = $this->excurl->reqAction('news/update', array_merge($_POST, array('ses_user_id' => $this->session->userdata('user_id'))), ['uploadfile']);
            $this->view(array('xcss' => $option->add_message->xcss, 'xmsg' => $option->message));
        } else {
            redirect('news');
        }
