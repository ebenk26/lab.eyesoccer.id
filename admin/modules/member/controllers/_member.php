...
class Member extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $session = array('xfield_' . $this->dtable => '', 'xsearch_' . $this->dtable => '', 'sortBy_' . $this->dtable => 'last_join',
        ...
        if ($this->input->post('val') == true) {
            ...
            if ($this->input->post('val') > 0) {
                ...
                if ($this->session->userdata('sortDir_' . $this->dtable) == 'asc' OR
                    ...
                    $session = array('xfield_' . $this->dtable => $this->session->userdata('xfield_' . $this->dtable), 'xsearch_' . $this->dtable => $this->session->userdata('xsearch_' . $this->dtable),
                                     'sortBy_' . $this->dtable => 'member_id', 'sortDir_' . $this->dtable => 'desc');
                }
            }
            $this->session->set_userdata($session);
            ...
            $data['dt'] = $this->excurl->reqCurl('member', $query)->data;
            $data['count'] = $this->excurl->reqCurl('member', $count)->data[0];
            $data['limit'] = $limit;
            $data['offset'] = $offset;
            $data['prefix'] = $this->dtable;
            $data['showpage'] = ceil($data['count']->cc / $limit);
    ...
    function search()
    {
        if ($this->input->post('val') == true) {
            ...
            $data['dt'] = $this->excurl->reqCurl('member', $query['query'])->data;
            $data['count'] = $this->excurl->reqCurl('member', $query['count'])->data[0];
            $data['limit'] = $limit;
            $data['offset'] = $this->offset;
            $data['prefix'] = $this->dtable;
    ...
    function save()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_created == 1) {
            ...
            $upload = $this->member_model->__upload($new_link);
            $key = substr(md5($this->library->app_key()), 0, 7);

            $cat = $this->excurl->reqCurl('member-category', ['member_type_id' => $this->input->post('category')])->data[0];
            $catsub = $this->excurl->reqCurl('member-category-sub', ['sub_member_id' => $this->input->post('subcategory')])->data[0];
            ...
            $dt1 = array(// General
                ...
                'category_member' => $this->input->post('recommended'),
                'url' => $new_link.'-'.$key,
                'pic' => $upload['data'],
                // Data
                'member_type' => $cat->member_type,
            ...
            if ($option['state'] == 0) {
                $this->member_model->__unlink($upload['data']);
    ...
    function edit($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            if ($id == '') {
                ...
                $query = array('eyemember_id' => $id, 'detail' => true);
                $ulevel = $this->library->user_check();
                if($ulevel->ff > 0)
                {
                ...
                $data['dt1'] = $this->excurl->reqCurl('member', $query)->data[0];
                $data['category'] = $this->excurl->reqCurl('member-category');
                $data['subcategory'] = $this->excurl->reqCurl('member-category-sub', ['category' => $data['dt1']->member_type]);
    ...
    function update()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            ...
            $option = $this->action->update(array('table' => $this->dtable, 'update' => $dt1,
                                                  'where' => array('eyemember_id' => $this->input->post('idx'))));
            if ($option['state'] == 0) {
                $this->member_model->__unlink($upload['data']);
            ...
            if ($this->input->post('member_pic') != '') {
                $this->member_model->__unlink($this->input->post('member_pic'));
            }
