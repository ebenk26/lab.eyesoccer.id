...
<div id='boxbutton'>
    <?php
        ...
        if(isset($_GET['id']))
        {
            ?> <a href="javascript:void(0)" id='button' onclick="actmenu('video/category/view<?php echo $sv->idback; ?>')">Back</a> <?php
        }
    ?>
    <a href="javascript:void(0)" id='button' onclick="openform('video/category/add<?php echo $sv->idstay; ?>')">Add New</a>
    
    <div class='search'>
        <label>Search By</label>
        <select name='cselect' class='cinput inselect'>
            <?php
                $field = array('category_name' => 'Name');
        ...
        <input type='text' name='csearch' class='cinput insearch' value='<?php echo $csearch; ?>' placeholder='Search...' onkeyup="actsearch('video/category/search<?php echo $sv->idstay; ?>')">
    </div>
...
<div id='boxaction'>
    ...
    <a href="javascript:void(0)" id='button' onclick="actcheck('video/category/checked')">Action</a>
    
    <div class='limit'>
        <label>Limit</label>
        <select name='climit' class='climit inselect' onchange="actlimit('video/category/view')">
            <?php
