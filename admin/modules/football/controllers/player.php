...
class Player extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $raccess = $this->library->role_access('football/player');
        if (isset($raccess)) {
            $this->roles = $raccess;
        }
    }
    ...
    function pagetable()
    {
        if ($this->input->post('val') == true) {
            ...
            $html = $this->load->view($this->config->item('base_theme') . '/player/player_table', $data, true);
    ...
    function add()
    {
        if ($this->roles == 'admin' OR $this->roles->menu_created == 1) {
            ...
            $data['position'] = $this->excurl->reqCurl('player-position');
            $data['foot'] = $this->excurl->reqCurl('player-foot');
    ...
    function edit($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            if ($id == '') {
                ...
                $data['dt1'] = $this->excurl->reqCurl('profile', $query)->data[0];
                $data['position'] = $this->excurl->reqCurl('player-position');
                $data['foot'] = $this->excurl->reqCurl('player-foot');
                $data['level'] = $this->excurl->reqCurl('player-level');
    ...
    function update()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            $option = $this->excurl->reqAction('football/player/update', array_merge($_POST, array('ses_user_id' => $this->session->userdata('user_id'))), ['uploadfile']);
            $this->view(array('xcss' => $option->add_message->xcss, 'xmsg' => $option->message));
        } else {
            redirect('football/player');
        }
    ...
    function delete($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_deleted == 1) {
            if ($id == '') {
                ...
                if ($this->input->post('val') == true) {
                    $option = $this->player_model->__delete($id);
        ...
        if ($this->input->post('checked') != NULL) {
            ...
            switch ($id) {
                case 1:
                    if ($this->roles == 'admin' OR $this->roles->menu_deleted == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->player_model->__delete($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
                ...
                case 2:
                    if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->player_model->__enable($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
                ...
                case 3:
                    if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->player_model->__disable($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
