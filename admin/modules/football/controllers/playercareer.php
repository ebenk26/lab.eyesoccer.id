...
class Playercareer extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $raccess = $this->library->role_access('football/playercareer');
        if (isset($raccess)) {
            $this->roles = $raccess;
        }
    }
    ...
    {
        $data['title'] = 'Player Career';
        $data['parent'] = $this->mparent;
        $data['roles'] = $this->roles;
        $data['content'] = $this->config->item('base_theme') . '/playercareer/playercareer';
        ...
        $data['dt'] = $this->excurl->reqCurl('player-career', $query)->data;
        $data['count'] = $this->excurl->reqCurl('player-career', array_merge($query, array('count' => true)))->data[0];
        $data['limit'] = $limit;
    ...
    function view($option = array())
    {
        if ($this->input->post('val') == true) {
            ...
            $data['dt'] = $this->excurl->reqCurl('player-career', $query)->data;
			$data['count'] = $this->excurl->reqCurl('player-career', $count)->data[0];
            $data['limit'] = $limit;
            $data['offset'] = $offset;
            $data['prefix'] = $this->dtable;
            $data['showpage'] = ceil($data['count']->cc / $limit);
            ...
            if ($this->input->post('val') > 0 OR isset($option['is_check'])) {
                $html = $this->load->view($this->config->item('base_theme') . '/playercareer/playercareer_jquery', $data, true);
            } else {
                $html = $this->load->view($this->config->item('base_theme') . '/playercareer/playercareer', $data, true);
            }
    ...
    function add()
    {
        if ($this->roles == 'admin' OR $this->roles->menu_created == 1) {
            $data['title'] = 'Player Career';
            $data['parent'] = $this->mparent;
            $data['content'] = $this->config->item('base_theme') . '/playercareer/add_playercareer';
            ...
            if ($this->input->post('val') == true) {
                $this->load->view($this->config->item('base_theme') . '/playercareer/add_playercareer', $data);
            } else {
                $this->load->view($this->config->item('base_theme') . '/template', $data);
            }
        } else {
    ...
    function save()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_created == 1) {
            $option = $this->excurl->reqAction('football/playercareer/save/?id=' . $_GET['id'], array_merge($_POST, array('ses_user_id' => $this->session->userdata('user_id'))));
    ...
    function edit($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            if ($id == '') {
                ...
                $data['title'] = 'Player Career';
                $data['parent'] = $this->mparent;
                $data['content'] = $this->config->item('base_theme') . '/playercareer/edit_playercareer';
                ...
                $data['dt1'] = $this->excurl->reqCurl('player-career', $query)->data[0];

                if ($this->input->post('val') == true) {
                    $this->load->view($this->config->item('base_theme') . '/playercareer/edit_playercareer', $data);
                } else {
                    $this->load->view($this->config->item('base_theme') . '/template', $data);
                }
            }
    ...
    function update()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            $option = $this->excurl->reqAction('football/playercareer/update/?id=' . $_GET['id'], array_merge($_POST, array('ses_user_id' => $this->session->userdata('user_id'))));
            $this->view(array('xcss' => $option->add_message->xcss, 'xmsg' => $option->message));
        } else {
            redirect('football/playercareer');
        }
    ...
    function delete($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_deleted == 1) {
            if ($id == '') {
                ...
                if ($this->input->post('val') == true) {
                    $option = $this->playercareer_model->__delete($id);
        ...
        if ($this->input->post('checked') != NULL) {
            ...
            switch ($id) {
                case 1:
                    if ($this->roles == 'admin' OR $this->roles->menu_deleted == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->playercareer_model->__delete($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
                ...
                case 2:
                    if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->playercareer_model->__enable($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
                ...
                case 3:
                    if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->playercareer_model->__disable($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
