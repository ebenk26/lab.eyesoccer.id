...
class Position extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $raccess = $this->library->role_access('football/position');
        if (isset($raccess)) {
            $this->roles = $raccess;
        }
    }
    ...
    public function index()
    {
        ...
        $data['content'] = $this->config->item('base_theme') . '/position/position';
        ...
        $data['dt'] = $this->excurl->reqCurl('player-position', $query)->data;
        $data['count'] = $this->excurl->reqCurl('player-position', array_merge($query, array('count' => true)))->data[0];
    ...
    function view($option = array())
    {
        if ($this->input->post('val') == true) {
            ...
            if ($this->input->post('val') > 0) {
                ...
                if ($this->session->userdata('sortDir_' . $this->dtable) == 'asc' OR
                    $this->session->userdata('sortDir_' . $this->dtable) == 'desc'
                        ...
                        $id = ($this->session->userdata('sortBy_' . $this->dtable) == '') ? 'id_position' : $this->session->userdata('sortBy_' . $this->dtable);
            ...
            $data['dt'] = $this->excurl->reqCurl('player-position', $query)->data;
            $data['count'] = $this->excurl->reqCurl('player-position', $count)->data[0];
            ...
            if ($this->input->post('val') > 0 OR isset($option['is_check'])) {
                $html = $this->load->view($this->config->item('base_theme') . '/position/position_jquery', $data, true);
            } else {
                $html = $this->load->view($this->config->item('base_theme') . '/position/position', $data, true);
            }
    ...
    function search()
    {
        if ($this->input->post('val') == true) {
            ...
            $data['dt'] = $this->excurl->reqCurl('player-position', $query['query'])->data;
            $data['count'] = $this->excurl->reqCurl('player-position', $query['count'])->data[0];
    ...
    function pagetable()
    {
        if ($this->input->post('val') == true) {
            ...
            $html = $this->load->view($this->config->item('base_theme') . '/position/position_table', $data, true);
    ...
    function add()
    {
        if ($this->roles == 'admin' OR $this->roles->menu_created == 1) {
            $data['title'] = 'Position';
            $data['parent'] = $this->mparent;
            $data['content'] = $this->config->item('base_theme') . '/position/add_position';

            if ($this->input->post('val') == true) {
                $this->load->view($this->config->item('base_theme') . '/position/add_position', $data);
            } else {
                $this->load->view($this->config->item('base_theme') . '/template', $data);
            }
        } else {
    ...
    function save()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_created == 1) {
            ...
            $option = $this->excurl->reqAction('football/position/save', $_POST);
    ...
    function edit($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
            if ($id == '') {
                ...
                $data['title'] = 'Position';
                $data['parent'] = $this->mparent;
                $data['content'] = $this->config->item('base_theme') . '/position/edit_position';
                $data['dt1'] = $this->excurl->reqCurl('player-position', ['id_position' => $id])->data[0];

                if ($this->input->post('val') == true) {
                    $this->load->view($this->config->item('base_theme') . '/position/edit_position', $data);
                } else {
                    $this->load->view($this->config->item('base_theme') . '/template', $data);
                }
            }
    ...
    function update()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                   $option = $this->excurl->reqAction('football/position/update', $_POST);
                   $this->view(array('xcss' => $option->add_message->xcss, 'xmsg' => $option->message));
                        } else {
            redirect('football/position');
        }
    ...
    function delete($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_deleted == 1) {
            if ($id == '') {
                ...
                if ($this->input->post('val') == true) {
                    $option = $this->position_model->__delete($id);
        ...
        if ($this->input->post('checked') != NULL) {
            ...
            switch ($id) {
                case 1:
                    if ($this->roles == 'admin' OR $this->roles->menu_deleted == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->position_model->__delete($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
                ...
                case 2:
                    if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->position_model->__enable($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
                ...
                case 3:
                    if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->position_model->__disable($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
