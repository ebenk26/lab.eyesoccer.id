...
class Playergallery extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $raccess = $this->library->role_access('football/playergallery');
        if (isset($raccess)) {
            $this->roles = $raccess;
        }
    }
    ...
    {
        $data['title'] = 'Player Gallery';
        $data['parent'] = $this->mparent;
        $data['roles'] = $this->roles;
        $data['content'] = $this->config->item('base_theme') . '/playergallery/playergallery';
    ...
    function view($option = array())
    {
        if ($this->input->post('val') == true) {
            ...
            if ($this->input->post('val') > 0 OR isset($option['is_check'])) {
                $html = $this->load->view($this->config->item('base_theme') . '/playergallery/playergallery_jquery', $data, true);
            } else {
                $html = $this->load->view($this->config->item('base_theme') . '/playergallery/playergallery', $data, true);
            }
    ...
    function pagetable()
    {
        if ($this->input->post('val') == true) {
            ...
            $html = $this->load->view($this->config->item('base_theme') . '/playergallery/playergallery_table', $data, true);
    ...
    function add()
    {
        if ($this->roles == 'admin' OR $this->roles->menu_created == 1) {
            $data['title'] = 'Player Gallery';
            $data['parent'] = $this->mparent;
            $data['content'] = $this->config->item('base_theme') . '/playergallery/add_playergallery';
            ...
            if ($this->input->post('val') == true) {
                $this->load->view($this->config->item('base_theme') . '/playergallery/add_playergallery', $data);
            } else {
                $this->load->view($this->config->item('base_theme') . '/template', $data);
            }
        } else {
    ...
    function save()
    {
        if ($this->input->post('val') == true AND $this->roles == 'admin' OR $this->roles->menu_created == 1) {
            $option = $this->excurl->reqAction('football/playergallery/save/?id=' . $_GET['id'], array_merge($_POST, array('ses_user_id' => $this->session->userdata('user_id'))));
    ...
    function delete($id = '')
    {
        if ($this->roles == 'admin' OR $this->roles->menu_deleted == 1) {
            if ($id == '') {
                ...
                if ($this->input->post('val') == true) {
                    $option = $this->playergallery_model->__delete($id);
        ...
        if ($this->input->post('checked') != NULL) {
            ...
            switch ($id) {
                case 1:
                    if ($this->roles == 'admin' OR $this->roles->menu_deleted == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->playergallery_model->__delete($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
                ...
                case 2:
                    if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->playergallery_model->__enable($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
                ...
                case 3:
                    if ($this->roles == 'admin' OR $this->roles->menu_updated == 1) {
                        for ($i = 0; $i < $count; $i++) {
                            $option = $this->playergallery_model->__disable($split[$i]);
                        }
                    } else {
                        $this->library->role_failed();
                    }
