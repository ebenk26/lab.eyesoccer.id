...
<div id='boxbutton'> 
    <a href="javascript:void(0)" id='button' onclick="openform('event/add')">Add New</a>
    
    <div class='search'>
        <label>Search By</label>
        <select name='cselect' class='cinput inselect'>
            <?php
                $field = array('search' => 'Title'/*, 'event_type' => 'Category'*/);
        ...
        <input type='text' name='csearch' class='cinput insearch' value='<?php echo $csearch; ?>' placeholder='Search...' onkeyup="actsearch('event/search')">
    </div>
...
<div id='boxaction'>
    ...
    <a href="javascript:void(0)" id='button' onclick="actcheck('event/checked')">Action</a>
    
    <div class='limit'>
        <label>Limit</label>
        <select name='climit' class='climit inselect' onchange="actlimit('event/view')">
            <?php
...
<div id='boxjq'>
    <div id='boxtable'>
        <?php $this->load->view($this->config->item('base_theme').'/event/event_table'); ?>
    </div>
    ...
    <?php
        if($showpage > 1)
        {
            echo "<div id='pageself'>
                    <div id='showurl' value='event/pagetable'></div>
