<table id='routertable'>
    ...
    <?php
        if ($count->cc > 0) {
            $i = $offset;
            foreach ($dt as $r) {
                $pic_a = $this->library->picUrl($r->logo_a, $r->url_logo_a, FDEYEVENT, 'small');
                $pic_b = $this->library->picUrl($r->logo_b, $r->url_logo_b, FDEYEVENT, 'small');

                echo "<tr>";
                echo "<td class='center'><input type='checkbox' name='selected[]' value='" . $r->id_event . "' class='ctab'></td>";
                echo "<td><span>" . $r->event . "</span></td>";
                echo "<td class='center hd-mobile'><div class='picproduct'><span style='background-image: url(" . $pic_a . ");'></span></div></td>";
                echo "<td class='center'>" . $r->team_a . "</td>";
                echo "<td class='center'>" . $r->score_a . " - " . $r->score_b . "</td>";
                echo "<td class='center'>" . $r->team_b . "</td>";
                echo "<td class='center hd-mobile'><div class='picproduct'><span style='background-image: url(" . $pic_b . ");'></span></div></td>";
