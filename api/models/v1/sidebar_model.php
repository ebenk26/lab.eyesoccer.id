...
class Sidebar_model extends CI_Model {
    ...
    function __sidebar($type = '', $lang = '', $pid = 0)
    {
	$query = array('is_active' => 1, 'lang_code' => $lang, 'sortBy' => 'sidebar_order', 'sortDir' => 'asc');
	if($type)
	{
	    $query = array_merge($query, array('sidebar_posisition' => $type));
	}
	
	$dt = $this->catalog->get_sidebar($query);
	if($dt)
	{
	    $dtData = [];
	    foreach($dt as $r)
