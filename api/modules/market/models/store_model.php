...
class Store_model extends CI_Model
{
    ...
    {
        $dt = $this->excurl->reqCurl('market', ['id' => $id])->data[0];
    ...
    function __upload($newname = '')
    {
        ...
        if ($this->input->post('pic') != '') {
            ...
            if ($this->input->post('temp_pic') != '') {
                $files = $this->input->post('temp_pic');
                $this->uploader->__unlink($path, $files);
            }
        }

        $upload = $this->uploader->multi_upload($path['config'], 'uploadfile', $path['path'], $pic, $newname);
